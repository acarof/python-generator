...
def find_crystal_bb( abc, start, length, vector, radius, has_atoms = False, radius_aom = 0.0, psf_file = None, xyz_file = None):
	if has_atoms:
	...
	if has_atoms:
		...
		active = [ (_[0]+1) for _ in enumerate(active) if _[1] == True]
		print 'Active molecule indices (1-based): ' + ' '.join(map(str, active))
		return active
	else:
