...
def main(inputs, paths):
    ...
    task = {
        'KIND_RUN' : 'TASK143-10times-100ps',
        'STEPS'    : 100,
        'SELECT'   : 'T',
        'FIRST_ADIABAT' : 1,
        'REPEAT'        : 2
    ...
    list_propagation = ['BORN_OPPENHEIMER', 'FSSH']
    list_collapse =    ['T']
    list_first_diabat = [1]
    list_rescaling    = ['NACV']
    list_nacv         = [ 'TOTAL']
    ...
    for system in systems:
        ...
        for dict in mega_list:
            ...
            for run in range(1, inputs.get('REPEAT') + 1):
                config = Config( inputs, paths, INIT = run, **dict)
